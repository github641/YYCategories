...
/**传入data，把plist文件的data，转换为数组，如果无法转成数组格式，那么返回nil
...
/**与NSArray中的arrayWithPlistData中方法的区别是“NSPropertyListSerialization propertyListWithData”方法调用中的枚举参数不同，一个是可变的NSPropertyListMutableContainersAndLeaves，一个是不可变的NSPropertyListImmutable。
 Creates and returns an array from a specified property list data.
